...
public class OrderDao extends RdbmsDao {
	...
	public Order selectOrder(int member_no) throws Exception {
		...
		try {
			...
			while (rs.next()) {
				...
				int i_no = rs.getInt(6);
				int jd_quantity = rs.getInt(7);
				int jd_tot = rs.getInt(8);
				odMap.put(i_no, new OrderDetail(jd_no, i_no, jd_quantity, jd_tot, j_no));
			}
	...
	public Order selectOrderByI_no(int item_no) throws Exception {
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		Order order = null;
